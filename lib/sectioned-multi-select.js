...
const addTag = (addItemButtonText, onCreateItem, searchTerm) => {
  return <TouchableOpacity style={{ padding: 10, marginTop: 10, backgroundColor: '#f2f2f2' }}
    onPress={() => onCreateItem(searchTerm)}>
    <Text style={{ textAlign: 'center' }}>{addItemButtonText}</Text>
  </TouchableOpacity>
}
                  ...
                  callIfFunction(loadingComponent)
                    ...
                    <View>
                      ...
                      {items && renderItems && renderItems.length ? (
                        ...
                        (!this.props.allowCreateItem) ? callIfFunction(noResultsComponent)
                          : callIfFunction(addTag(this.props.addItemButtonText, this.props.onCreateItem, searchTerm))
                      ) : null}
                    </View>
                  )}
              </View>
